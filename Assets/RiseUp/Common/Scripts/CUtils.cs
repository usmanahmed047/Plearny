...
public class CUtils
{
    ...
    public static void OpenStore()
    {
		...
		Application.OpenURL("https://itunes.apple.com/app/id" + ConfigController.Config.macAppID);
#endif
    }
    ...
    {
        return CPlayerPrefs.GetInt("player_type", 0);
    }

    public static void SetPlayerType(int type)
    {
        CPlayerPrefs.SetInt("player_type", type);
    }

    public static bool IsPlayerUnlock(int type)
    {
        if (type == 0) return true;
        return CPlayerPrefs.GetBool("unlocked_type_" + type, false);
    }

    public static void SetPlayerUnlock(int type)
    {
        CPlayerPrefs.SetBool("unlocked_type_" + type, true);
    }
